...
// Interface functions
static bool btdrv_open()
{
  uart_fd = open("/dev/brcm_bt_drv", W_OK);

  if ( uart_fd >= 0 ) {
    LOG_DEBUG(LOG_TAG, "%s uart_fd = %d", __func__, uart_fd);
    return true;
  } else {
    LOG_DEBUG(LOG_TAG, "%s error", __func__);
    return false;
  }
...
static bool hal_open() {
  ...
  eager_reader_register(uart_stream, thread_get_reactor(thread), event_uart_has_bytes, 0);
...
const hci_hal_t *hci_hal_v4l2_get_interface() {
  vendor = vendor_get_interface();
  return &interface;
}

const hci_hal_t *hci_hal_v4l2_get_test_interface(vendor_t *vendor_interface) {
